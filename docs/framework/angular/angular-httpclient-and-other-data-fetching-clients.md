...
- When using SSR, `HttpClient` will [cache requests](https://angular.dev/guide/ssr#caching-data-when-using-HttpClient) performed on the server. This will prevent unneeded requests on the client, but will not avoid an initial loading state when the HttpClient has not been called yet on the client. TanStack Query can still [hydrate its cache](./guides/ssr.md) from the server-rendered HTML when you use `provideTanStackQuery`. See the [SSR guide](./guides/ssr.md) for more information.
